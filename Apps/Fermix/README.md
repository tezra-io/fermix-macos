...
The macOS app for Fermix. Today it ships the Realtime voice companion; the
unified menu bar surface is being built on the same package.
...
- `Fermix` — the GUI executable. Its `main.swift` calls `FermixApp.main()` and
  nothing else. Owns the microphone.
...
Use the script instead of `swift run Fermix` for normal GUI testing. It builds
with SwiftPM under `~/Library/Caches/io.tezra.FermixPet`, stages a proper
`.app` bundle, and installs it to `~/Applications` when run with `install`.
Dock, Quit, microphone permissions, and app identity all come from
`Product.json` by way of `scripts/render_info_plist.sh`; the bundle identifier
stays `io.tezra.FermixPet` so the existing microphone grant survives.

On first listen, macOS should prompt for microphone access. If it was denied
previously, enable `Fermix` in System Settings -> Privacy & Security ->
