...
**The dev loop stages a different app.** `scripts/dev_e2e.sh` exports `PRODUCT_CONFIG_OVERLAY=scripts/product.dev.json`, a development identity applied over `Product.json` at staging time and never checked into it: `Fermix Dev.app`, bundle identifier `io.tezra.FermixPet.dev`, agent label `io.tezra.FermixPet.dev.agent`, url scheme `fermix-dev`, and the support folder `Fermix Dev`. macOS keys the launchd job on the label, the login item, the TCC grants and the copy count on the identifier, and there is one `launcher.json` per support folder, so that overlay is what lets a dev bundle run beside an installed `Fermix.app`: each has its own agent, its own grants and its own bootstrap record, and the dev loop never touches the installed app's. The overlay also declares an updater the bundle cannot configure, so a development bundle is offered no update and the release audience of `verify_staged_app.sh` refuses one outright. The release path sets no overlay and is unchanged: `package_release.sh` stages `Fermix.app` from `Product.json` exactly as before.
...
- The first two releases cannot take an update through the app, and no feed fixes that. The GUI read its bundled engine manifest from the engine slot itself, while `stage_app.sh` writes one release tree per architecture beneath that slot, so the manifest it opened was never there: the launch reconcile answered aligned with nothing to compare, and `Check for updates` refused every offer with an unknown bundled engine. It is fixed in the release after 0.1.1, and reaching that release takes one manual reinstall on every Mac that already carries 0.1.0 or 0.1.1: the DMG from the GitHub release, or `brew upgrade --cask fermix`. Updates through the app work from that copy onwards. `docs/design/RCA_2026-09-12_UPDATE_REFUSED_BUNDLED_ENGINE_UNKNOWN.md` carries the analysis.
...
- A cask upgrade is the update path nothing in the app drives, and it replaces the bundle under a registered agent. `brew upgrade --cask fermix` swaps `/Applications/Fermix.app` while the agent is registered and running, and the launchd job survives it with a stale parent bundle version: macOS marks the job as needing an LWCR update, and in that state it spawns the agent with no default environment at all, so the launcher refuses every start for want of a PATH. Three things answer it. The bundled agent plist declares its own PATH, from `Product.json`, so a spawn that inherits nothing still has one. An enable unregisters before it registers, so turning the switch on rebuilds the job rather than confirming a degraded one. And the registration receipt in `launcher.json` records the app build that wrote it, so the first launch by a new build rebuilds the registration once and records the new receipt; a receipt with no build, which every install made before this version carries, counts as another build and heals on first launch. The in-app Sparkle path is unaffected and still owns its own transaction: it re-registers through the launch reconcile, which writes this build's receipt, and the launch rebuild then finds nothing to do.
...
One more row, and it is the one the in-app update path cannot stand in for: **`brew upgrade --cask over a running service`**. With the background service registered and the daemon answering, upgrade the cask, then open the app and confirm without touching the switch that the daemon answers, that `launchctl print gui/$UID/io.tezra.FermixPet.agent` reports the new bundle version and no pending LWCR update, and that the upgrade log carries a line per registration from `Fermix --unregister-login-items`. Walk it before announcing.
